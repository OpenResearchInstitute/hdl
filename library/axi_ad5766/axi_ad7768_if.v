...
module ad7768_if #(
  ...
  output      [ 31:0]     adc_status_0,
  output      [ 31:0]     adc_status_1,
  output      [ 31:0]     adc_status_2,
  output      [ 31:0]     adc_status_3,
  output      [ 31:0]     adc_status_4,
  output      [ 31:0]     adc_status_5,
  output      [ 31:0]     adc_status_6,
  output      [ 31:0]     adc_status_7,
  output      [  7:0]     adc_crc_ch_mismatch,
  // control interface

  input                   adc_sshot,
  input       [ 1:0]      adc_format,
...
// new added 
  ...
  assign adc_crc_mismatch_s[4] = (adc_crc_read_data[4] == adc_crc_s[4] && NUM_CHANNELS == 8) ? 1'b0 : adc_crc_enable;
  assign adc_crc_mismatch_s[5] = (adc_crc_read_data[5] == adc_crc_s[5] && NUM_CHANNELS == 8) ? 1'b0 : adc_crc_enable;
...
always @(posedge adc_clk) begin
 ...
 always @(posedge adc_clk) begin
    if (adc_valid_p == 1'b1) begin
      ...
      end else if( adc_format == 'h1 || (adc_format == 'h0 && NUM_CHANNELS == 4 )) begin   // 2 active lines
